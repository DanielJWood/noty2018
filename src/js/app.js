...
var round61Names = [
  ["Jimbob Ghostkeeper","Dr. Narwhals Mating",[2,1]]  
]
...
d3.queue()
    ...
    .defer(d3.csv, "https://s3-us-west-2.amazonaws.com/energy2/social/round6_1.csv")    
    .await(ready);

function ready(error, fruithandler, bulltron, dragonwagon, chrotchtangle, round2_1, round2_2,round3_1,round4_1,round5_1,round6_1) {

  var timetime = document.getElementById("time")
  const parseTime = d3.timeParse("%m/%d/%y %H:%M")
  const formatTime = d3.timeFormat("%H:%M (EDT), %B %d, %Y");

  timetime.innerHTML = formatTime(d3.timeHour.offset(parseTime(round6_1[round6_1.length - 1].date),-4));

  var dataRollup = compileData(fruithandler, bulltron, dragonwagon, chrotchtangle, round2_1, round2_2,round3_1,round4_1,round5_1,round6_1);
  ...
  const myTunaGod2 = new plusminus();
  // Declare our charts

  myTunaGod2.create('#tunagod2', round6_1, {
    // This is where you would overwrite props to change the name of the data to match your unique data (in this case multipleLine)
    // See above in single chart for changing the props
    yName: d => round61Names[0][0],
    y2Name: d => round61Names[0][1],
    rank: d => round61Names[0][0],
    yAccessor: d => (d[round61Names[0][0]] == undefined) ? -10 : +d[round61Names[0][0]].replace(/,/g, ""),
    y2Accessor: d => (d[round61Names[0][1]] == undefined) ? -10 : +d[round61Names[0][1]].replace(/,/g, "")
  });

  const myRound610 = new chart();
  ...
  myRound610.create('#round6-1-0', round6_1, {
    // This is where you would overwrite props to change the name of the data to match your unique data (in this case multipleLine)
    // See above in single chart for changing the props
    yName: d => round61Names[0][0],
    y2Name: d => round61Names[0][1],
    rank: d => round61Names[0][2],
    yAccessor: d => (d[round61Names[0][0]] == undefined) ? -10 : +d[round61Names[0][0]].replace(/,/g, ""),
    y2Accessor: d => (d[round61Names[0][1]] == undefined) ? -10 : +d[round61Names[0][1]].replace(/,/g, "")
  });
...
function compileData(a,b,c,d,e,f,g,h,aa,ab) {
  var allData = [a,b,c,d,e,f,g,h,aa,ab]
